...
- 🔌 **扩展控制**: 为每个配置文件单独开关 `--disable-extensions`，可选择是否加载浏览器扩展（默认禁用以保持纯净环境）。
- 🐞 **调试器模式**: 可选启用 Chrome 远程调试功能，支持自定义启动端口和转发端口，为 Puppeteer/Playwright 等自动化工具暴露 CDP 接口。
...
- **远程自动化**: 在创建或编辑配置时，开启 **调试器模式**，并设置 **启动端口**（对应 Chrome 的 `--remote-debugging-port`）以及可选的 **转发端口**（监听 `0.0.0.0`）。这允许您通过局域网内的其他设备使用 Puppeteer 或 Playwright 访问转发端口来远程控制浏览器实例。
- **扩展开关**: 在配置表单中，使用 **禁用扩展** 开关控制 Chrome 是否加载扩展。当您需要在会话中使用已安装的扩展时，请将其关闭。
